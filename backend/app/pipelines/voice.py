...
def load_voice_pools(path: str) -> Dict[str, List[Dict[str, Any]]]:
    """
    Supports TWO formats:
    
    NEW FORMAT (ElevenLabs API response):
      { "voices": [{"voice_id": "...", "name": "...", "labels": {...}, ...}] }
    ...
    def clean(items: List[Any]) -> List[Dict[str, Any]]:
        out = []
        seen = set()
        for x in items or []:
            if isinstance(x, str):
    ...
    if isinstance(data, dict) and "voices" in data:
        ...
        for voice in data.get("voices", []):
            ...
            if gender == "male":
                male_objs.append(voice)
            elif gender == "female":
                female_objs.append(voice)
            # Ignore "neutral" or other genders for now
        
        return {"male": clean(male_objs), "female": clean(female_objs)}
...
def fetch_available_voice_ids(api_key: str) -> Set[str]:
    ...
    """
    print(f"[voice] Fetching available voices from ElevenLabs API...")
    r = requests.get(
    ...
    print(f"[voice] API response status: {r.status_code}")
    r.raise_for_status()
    data = r.json()
    voices = data.get("voices") or []
    print(f"[voice] Found {len(voices)} voices from API")
    
    voice_ids = {v.get("voice_id") for v in voices if v.get("voice_id")}
    print(f"[voice] Available voice IDs: {sorted(list(voice_ids))}")
...
def generate_voice(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # ✅ Optional bypass for debugging (set SKIP_VOICE_VALIDATION=true in .env)
    skip_validation = os.getenv("SKIP_VOICE_VALIDATION", "").strip().lower() in {"1", "true", "yes"}
    if skip_validation:
        print("[voice] ⚠️ SKIP_VOICE_VALIDATION is enabled - bypassing voice ID validation!")
    ...
    if voice_mode == "female":
        ...
        if not skip_validation and female_voice_id not in available:
        ...
        if not skip_validation and male_voice_id not in available:
